from flask import Flask, request, url_for, session, redirect
import spotipy
from spotipy.oauth2 import SpotifyOAuth
import time

app = Flask(__name__)

app.secret_key = "pihfwaeiufopa278349hjiafawei"
app.config['SESSION_COOKIE_NAME'] = 'Wong Coookie'
TOKEN_INFO = "token_info"

@app.route('/')
def login():
    sp_oauth = create_spotify_oauth()
    auth_url = sp_oauth.get_authorize_url()
    return redirect(auth_url)

@app.route('/redirect') 
def redirect_page():  
    sp_oauth = create_spotify_oauth()
    session.clear()
    code = request.args.get('code')
    token_info = sp_oauth.get_access_token(code)
    session[TOKEN_INFO]=token_info
    return redirect(url_for('getTracks', _external=True))

@app.route('/getTracks')
@app.route('/getTracks')
@app.route('/getTracks')
def getTracks():
    try:
        token_info = get_token()
    except Exception as e:
        print(f"Error: {e}")
        return redirect("/")  # Redirect to login if token is missing

    sp = spotipy.Spotify(auth=token_info['access_token'])
    all_songs = []
    iter = 0

    while True:
        results = sp.current_user_saved_tracks(limit=50, offset=iter * 50)
        items = results.get("items", [])
        for item in items:
            track = item["track"]
            song_name = track["name"]
            artist_name = track["artists"][0]["name"]
            all_songs.append(f"{song_name} {artist_name}")

        iter += 1
        if len(items) < 50:
            break

    # Save to a CSV for downloading
    import pandas as pd
    df = pd.DataFrame(all_songs, columns=["column"])
    df.to_csv("songs.csv", index=False)

    return f"Saved {len(all_songs)} songs to songs.csv"


def get_token():
    token_info = session.get(TOKEN_INFO, None)
    if not token_info:
        raise Exception("No token found in session. Please log in again.")

    now = int(time.time())
    is_expired = token_info['expires_at'] - now < 60

    if is_expired:
        sp_oauth = create_spotify_oauth()
        token_info = sp_oauth.refresh_access_token(token_info['refresh_token'])
        session[TOKEN_INFO] = token_info  # Store the updated token

    return token_info


def create_spotify_oauth():
    return SpotifyOAuth(
        client_id="cb9b955dfc654282a9ec03777cb2dcab",
        client_secret="576c1af2e28149e1acb67c73a310c76a",
        redirect_uri=url_for('redirect_page', _external=True),  
        scope="user-library-read"
    )

if __name__ == '__main__':
    app.run(debug=True)  
